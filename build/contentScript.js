
/*global chrome*/

//Notify background when a text selection has been made
//background will save
function notifyBackground(selection) {

    console.log("notify background ");

    chrome.runtime.sendMessage({ selection }, function (response) {
        console.log(response);
    });
}

//Notify the JS running in the content script
function notifyApplication(selection) {

    window.postMessage({
        type: 'content_script_type',
        selection: selection
    },
        '*' /* targetOrigin: any */);
}

//Get the text selected
function getSelectionText() {
    var text = "";
    if (window.getSelection) {
        text = window.getSelection().toString();
    } else if (document.selection && document.selection.type !== "Control") {
        text = document.selection.createRange().text;
    }
    return text;
}


//Wait for message from the extension
chrome.runtime.onMessage.addListener(

    function (request, sender, sendResponse) {
      
        if (request.method === "toclip") {
            console.log("to clip")
            setClipboard("to clip");
            sendResponse({ status: 1 });
        }
        else
            sendResponse({});
    }
)

let isSelecting = false;

//Check if the user is selecting 
document.onselectionchange = () => {
    isSelecting = true
};

//When the user finish to select a text
window.onmouseup = (event) => {

    console.log(event)

    if (isSelecting) {

        if (getSelectionText()) {
            showPapyrus(
                function() {
                    notifyApplication(getSelectionText());
                }
            );
            // notifyBackground(getSelectionText());
            isSelecting = false;
        }
    }
}


var div = document.createElement('div');

div.setAttribute("id", "div-papyrus");
div.style.position = 'fixed';
div.style.top = '481px';
div.style.right = '100px  ';

document.body.appendChild(div);

function showButton() {

    let div = document.getElementById("div-papyrus");
    let img = document.createElement('img');
    img.src = "data:image/svg+xml;base64,"
    img.style.width = '30px';
    img.style.height = '30px';
    div.style.width = '30px';
    div.style.height = '30px';
    div.style.zIndex = 999999;

    div.appendChild(img);


}


function showPapyrus(fct) {

    let div = document.getElementById("div-papyrus");
    div.style.top = '0%';
    div.style.left = '0%';
    div.style.width = '100%';
    div.style.height = '100%';
    div.style.display = "block";
    div.style.zIndex = 999999;

    var s = document.createElement('script');
    s.src = chrome.runtime.getURL('script/papyrus.js');

    s.onload = function () {
        this.remove();
        fct();
    };

    div.appendChild(s);

    div.onclick = undefined;

}

div.onclick = function () {

    showPapyrus(
        function () {
            window.postMessage({
                type: 'content_script_type',
                list: true
            }, '*' /* targetOrigin: any */);
        }
    );


}

showButton();

